#!/bin/bash
# Based on steps/nnet/decode.sh and steps/decode_biglm.sh by Karel Vesely, Daniel Povey, revise the biglm decode script to fit for DNN model.
...
max_mem=2000000000 # approx. limit to memory consumption during minimization in bytes
...
if [ $# != 5 ]; then
   echo "Usage: $0 [options] <graph-dir> <old-LM-fst> <new-LM-fst> <data-dir> <decode-dir>"
...
newlm_cmd="fstproject --project_output=true $newlm_fst |fstarcsort --sort_type=ilabel |"
 ...
 if [ $stage -le 0 ]; then
   $cmd $parallel_opts JOB=1:$nj $dir/log/decode.JOB.log \
     ...
     latgen-biglm-faster-mapped$thread_string --verbose=5 --min-active=$min_active --max-active=$max_active --max-mem=$max_mem --beam=$beam \
     --lattice-beam=$lattice_beam --acoustic-scale=$acwt --allow-partial=true --word-symbol-table=$graphdir/words.txt \
     $model $graphdir/HCLG.fst "$oldlm_cmd" "$newlm_cmd" ark:- "ark:|gzip -c > $dir/lat.JOB.gz" || exit 1;
 fi
